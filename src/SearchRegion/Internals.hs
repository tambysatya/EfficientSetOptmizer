...
{-| TODO use a min heap -}
data SRUB = SRUB {_srMdl :: OptEff, _srUB :: ![UB]}
makeLenses ''SRUB
instance Show SRUB where show (SRUB _ sr) = show $ fmap _szU sr
...
-}
updateSR :: (MonadIO m) => GlobalBounds -> ExploredUB -> ProjDir -> Maybe Point -> SubOpt -> SRUBT m ()
updateSR gbnds zexp pdir Nothing _ = do
    sr <- use srUB
    srUB .= catMaybes (updateZoneNothing gbnds zexp pdir <$> sr)
updateSR gbnds zexp pdir@(ProjDir l) (Just pt) estimation@(SubOpt s) = do --SRUB mdl $ sr >>= updateZoneJustWithRR gbnds zexp hopt pdir lb pt estimation
        sr <- use srUB
        ret <- forM sr $ \u -> updateZoneJustWithRR gbnds zexp pdir lb_l pt estimation u
...
-- TODO strict evaluation
updateZoneJustWithRR :: (MonadIO m) => GlobalBounds -> ExploredUB -> ProjDir -> Double ->Point -> SubOpt -> UB -> SRUBT m [UB]
updateZoneJustWithRR gbnds zexp pdir lb_l pt estimation@(SubOpt cur) ub = do
            newzones <- updateZoneJustReopt gbnds ub pt
            ret <- forM newzones $ applyReductionRule zexp pdir lb_l estimation
            pure $ catMaybes ret
...
updateZoneJustReopt gbnds ub pt 
        | pt `domS` ub = do 
                lb <- zoom srMdl $ do
                           setLocalUpperBoundM ub
                           solveFromPointM pt
                           getObjValueM
                liftIO $ putStrLn $ "\t\t Splitting " ++ show ub ++ " lb=" ++ show lb
                pure $ catMaybes [child gbnds pt ub i & _Just.szLB %~ max (HyperOpt lb)  | i <- ChildDir <$> [1..p]]
        | pt `domL` ub = do -- pure [updateDefiningPoints pt ub]
                {-| Même quand on appartient pas à la zone strictement, on reoptimise [TODO] -}
                lb <- zoom srMdl $ do
                           reoptimizeFromM ub
                           solveFromPointM pt
                           getObjValueM
                liftIO $ putStrLn $ "\t\t Updating " ++ show ub ++ " lb=" ++ show lb
                pure [updateDefiningPoints pt ub & szLB %~ max (HyperOpt lb)]
...
deleteZone ub (SRUB mdl sr) = SRUB mdl $ L.deleteBy ((==) `on` toBound) ub sr
...
selectZone (SRUB _ sr) = ExploredUB $ L.maximumBy (compare `on` view szLB) sr -- selects the most promising zone (to find the optimum faster)

emptySR :: SRUB -> Bool
emptySR (SRUB _ sr) = null sr

srSize :: SRUB -> Int
srSize (SRUB _ sr) = length sr
